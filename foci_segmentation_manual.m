...
%     mask=read_mask(name_mask);
    ...
    for cell_num =1:size(bbs,1)
        ...
        bb=bbs(cell_num,:);
        shape=size(ab_uint_whole);
        q=round(bb(2):bb(2)+bb(5));
        qq=round(bb(1):bb(1)+bb(4));
        qqq=round(bb(3):bb(3)+bb(6));
        q(q<1)=[];
        q(q>shape(1))=[];
        qq(qq<1)=[];
        qq(qq>shape(2))=[];
        ...
        ab_uint = ab_uint_whole(q,qq,qqq);
