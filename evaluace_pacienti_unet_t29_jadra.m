...
for folder_num=1:length(folders)
    ...
    for img_num=1:length(names)
        ...
        nuc_b = regionprops3(L_nuc_mask,c,'MeanIntensity','MaxIntensity');
        nuc_b_mean = nuc_b.MeanIntensity;
        nuc_b_max = nuc_b.MaxIntensity;
        
        nuc_r = regionprops3(L_nuc_mask,a,'MeanIntensity','MaxIntensity');
        nuc_r_mean = nuc_r.MeanIntensity;
        nuc_r_max = nuc_r.MaxIntensity;
        
        nuc_g = regionprops3(L_nuc_mask,b,'MeanIntensity','MaxIntensity');
        nuc_g_mean = nuc_g.MeanIntensity;
        nuc_g_max = nuc_g.MaxIntensity;
        ...
        tmp=nuc_b_max(other_table.CellNum);
        other_table = addvars(other_table,tmp,'NewVariableNames','NucBMax');
        tmp=nuc_b_mean(other_table.CellNum);
        other_table = addvars(other_table,tmp,'NewVariableNames','NucBMean');

        tmp=nuc_r_max(other_table.CellNum);
        other_table = addvars(other_table,tmp,'NewVariableNames','NucRMax');
        tmp=nuc_r_mean(other_table.CellNum);
        other_table = addvars(other_table,tmp,'NewVariableNames','NucRMean');
        
        tmp=nuc_g_max(other_table.CellNum);
        other_table = addvars(other_table,tmp,'NewVariableNames','NucGMax');
        tmp=nuc_g_mean(other_table.CellNum);
        other_table = addvars(other_table,tmp,'NewVariableNames','NucGMean');
        ...
        res_table=[r_table,g_table,rg_table,shape_table,other_table,b_table];
