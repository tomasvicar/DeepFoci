function data = readFile(filename,type,imgs2read,norm,stats,whole)
    ...
    if whole
        ...
        postion_vec2 = 1:size_v(1);
    else
        tmp = randi([1 size_v(1)-patch_size(1)]);
        postion_vec1=tmp:tmp+patch_size(1);

        tmp = randi([1 size_v(2)-patch_size(2)]);
        postion_vec2=tmp:tmp+patch_size(2);
    end
