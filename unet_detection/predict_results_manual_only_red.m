...
load('test3_value_aug_mult_onlyr.mat')
...
%     save_features=strrep(name,'data_','features_window_');
    ...
    save_unet_foci_detection_res_only_red=strrep(name,'data_','unet_foci_detection_res_only_red');
    save_unet_foci_detection_res_only_red=strrep(save_unet_foci_detection_res_only_red,'.tif','.mat');
...
%     mask_points_foci2=imgaussfilt3(single(mask_points_foci),[2,2,1])*59.5238*10;
    ...
    vys=predict_by_parts_detection(a,a,c,net);

    save(save_unet_foci_detection_res_only_red,'vys')
