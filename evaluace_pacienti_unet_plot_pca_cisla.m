...
%                 roudness=(rV*12.57)./(rA);
                ...
                cell_nums_all=res_table.CellNum;
                cell_num_one=cell_nums_all(use_row);
                
                
                img_names_all=res_table.ImgName;
                img_names_one=img_names_all(use_row);
                
                
                if ~isempty(cell_num_one)
                    cell_num_one=cell_num_one(1);
                    img_names_one=img_names_one{1};
                    img_names=[img_names,img_names_one];
                    cell_num_in_img=[cell_num_in_img,cell_num_one];
                    drawnow;
                end
...
img_names=img_names';
cell_num_in_img=cell_num_in_img';
cislo_bunky=(1:length(img_names))';

% mkdir('../pca_cisla')
writetable(table(img_names,cell_num_in_img,cislo_bunky),'../pca_cisla/tabulka.xlsx')
...
for g_num = 1:length(gs)
    ...
    cislo_bunky_mixed=[cislo_bunky_mixed;cislo_bunky(tmp)];
end
...
tmp=cellfun(@num2str,num2cell(cislo_bunky_mixed),'UniformOutput',false);
tmp=tmp(ii(1:end-15));
figure('units','normalized','outerposition',[0 0 1 1])
subset=biplotG(coefs(:,1:2),s,'VarLabels',var_names,'Groups',g,'Format',format,'ObsLabels',tmp );
...
print_png_eps_svg_fig(['../pca_cisla/pca_biplot_' num2str(qq)])
