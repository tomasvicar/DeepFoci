...
%% setup
%takhle to nejdede
src_path = 'C:\Data\Vicar\foky_final_cleaning\FOR ANALYSIS\NANOREP_labeling_new_labeled_resaved';
dst_paht = 'C:\Data\Vicar\foky_final_cleaning\FOR ANALYSIS\NANOREP_labeling_new_labeled_resaved_prepared';


% load filenames for 53BP1, gH2AX and DAPI  - modify based on your data
names_53BP1 = subdir([src_path '/data_RAD51.tif']);
filenames.imgs_53BP1 = {names_53BP1(:).name};
filenames.imgs_gH2AX = cellfun(@(x) replace(x,'RAD51.tif','gH2AX.tif'),filenames.imgs_53BP1,'UniformOutput',false);
filenames.imgs_DAPI = cellfun(@(x) replace(x,'RAD51.tif','DAPI.tif'),filenames.imgs_53BP1,'UniformOutput',false);
json_labels = cellfun(@(x) replace(x,'data_RAD51.tif','labels.json'),filenames.imgs_53BP1,'UniformOutput',false);
...
for img_num = 1:length(filenames.imgs_53BP1)
    ...
    z_scale_factor = 2;
    lbls.points_53BP1_gH2AX_overlap = get_overlaped_points(lbls.points_RAD51,lbls.points_gH2AX);%%%%%% get overlaped points
