...
% jak?
% - pro každou buňku najít optimální úhel a ten použít
% - definovat track - jako 3  v řadě s nějakou odchylkou?
% - spočítat tracky a pozice mimo
% 
% 
...
%             channels_check = [channels_check;chanel_names(order)];
            ...
            binary_detection_rg_labled =  binary_detection_rg .* segmentation;

            num_cells = max(segmentation(:));
            for cell_num = 1:num_cells
        
                s = regionprops(binary_detection_rg_labled==cell_num,'centroid');
                detections = round(cat(1, s.Centroid));
                ...
                for angle = 0:3:359
                    ...
                    detections_rotated = detections * R;
                    
                    [~,sotr_ind] = sort(detections_rotated(:,2));
                    detections_rotated_sorted = detections_rotated(sotr_ind,:);
                    
                    detections_rotated_sorted_for_del = detections_rotated_sorted;
                    detections_rotated_sorted_for_del_ind =1:length(detections_rotated_sorted_for_del);
                    results = {};

                    actual = detections_rotated_sorted_for_del(1,:);
                    detections_rotated_sorted_for_del(1,:) = [];
                    result = detections_rotated_sorted_for_del_ind(1);
                    detections_rotated_sorted_for_del_ind(1) = [];
                    

                    while ~isempty(detections_rotated_sorted_for_del)

                        new_line = 0;
                        n = size(detections_rotated_sorted_for_del,1);
                        for row_num = 1:n 

                            cond1 = abs(detections_rotated_sorted_for_del(row_num,1) - actual(1)) < x_lim;
                            cond2 = abs(detections_rotated_sorted_for_del(row_num,2) - actual(2)) < y_lim;
                            if cond1 && cond2
                                detections_rotated_sorted_for_del(row_num,:) = [];
                                result = [result detections_rotated_sorted_for_del_ind(row_num)];
                                detections_rotated_sorted_for_del_ind(row_num) = [];
                                break
                            end
                            if row_num == n 
                                new_line = 1;
                        ...
                        if isempty(detections_rotated_sorted_for_del)
                            results = [results, result];
                        end
                    ...
                    for kk = 1:length(results)
                        restult = results{kk};
                        results_orig = [results_orig,{sotr_ind(restult)}];
                    end


                    results_all = [results_all,{results_orig}];
                    drawnow;
    
                end

                in_line = zeros(1,length(results_all));
                for k = 1:length(results_all)
                    ...
                    for kk = 1:length(results)
                        restult = results{kk};
                        if length(restult) >= 3
                            count = count + length(restult);
                        end
                    end
                    in_line(k) = count;
                end
                [~,best_results_ind] = max(in_line) ;
                best_results = results_all{best_results_ind};


                hold on
                for kk = 1:length(best_results)
                    restult = best_results{kk};
                    if length(restult) >= 3
                        plot(detections(restult,1),detections(restult,2),'oy')
                        plot(detections(restult,1),detections(restult,2),'y')
                    end
                end
                drawnow;
